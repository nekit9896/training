"""
Конфигурация тестового набора Real_73_tn3_39km_56
Особенности набора:
- Режим МТ стационар
- Одна утечка на координате 40 км
- Объём утечки 80 м³
- Интенсивность утечки 1,8%
- Утечка завершается спустя 7 минут после обнаружения, в связи с этим сдвинуты offset тестов
"""
...
# ===== Константы набора =====
SUITE_NAME = "Real_73_tn3_39km_56"
SUITE_DATA_ID = 14
...
# Параметры утечки
LEAK_COORDINATE_METERS = 39000.0
LEAK_VOLUME_M3 = 56.8
...
# ===== Конфигурация набора =====
REAL_73_CONFIG = SmokeSuiteConfig(
    # ===== Метаданные =====
    ...
    controlled_sites_with_segment=CaseData(
        expected_result={
            ...
            "exp_novovelichkovskaya_krymskaya": (StationaryStatus.UNSTATIONARY, LdsStatus.SERVICEABLE),
            "exp_krymskaya_grushovaya": (StationaryStatus.UNSTATIONARY, LdsStatus.DEGRADATION),
            "exp_backup_route_bejsug": (StationaryStatus.STOPPED, LdsStatus.FAULTY),
            "exp_backup_route_ponura": (StationaryStatus.STOPPED, LdsStatus.SERVICEABLE),
    ...
    leak=LeakTestConfig(
        ...
        balance_algorithm_leak_waiting_test=CaseMarkers(test_case_id="175", offset=43),  # Длительность теста 5 минут
        balance_algorithm_leak_detected_test=CaseMarkers(test_case_id="177", offset=54),
        possible_leak_in_journal_test=CaseMarkers(test_case_id="211", offset=48),
        # ----- Тест AllLeaksInfo -----
        all_leaks_info_test=CaseMarkers(test_case_id="14", offset=54),
        # ----- Тест LeaksContent -----
