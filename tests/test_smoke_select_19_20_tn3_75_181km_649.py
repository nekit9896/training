...
async def test_main_page_info_unstationary(ws_client, cfg):
    ...
    # Для этого теста нужно проверить переход в UNSTATIONARY
    # Используем ту же базовую функцию, но cfg содержит expected_stationary
    await _test_main_page_info(ws_client, cfg)
...
async def test_all_leaks_info(ws_client, cfg):
    """[AllLeaksInfo] Проверка сообщения об утечке (Утечка 1: 75км)"""
...
async def test_tu_leaks_info(ws_client, cfg):
    """[TuLeaksInfo] Проверка сообщения об утечке типа TuLeaksInfoContent (Утечка 1)"""
...
async def test_output_signals(ws_client, cfg):
    """[OutputSignalsInfo] Проверка выходных сигналов (Утечка 1: 75км)"""
    await _test_output_signals(ws_client, cfg)


@pytest.mark.parametrize("cfg", [make_test_param(CONFIG, "all_leaks_info_leak_2")])
async def test_all_leaks_info_leak_2(ws_client, cfg):
    """[AllLeaksInfo] Проверка сообщения об утечке (Утечка 2: 181км)"""
    await _test_all_leaks_info_leak_2(ws_client, cfg)


@pytest.mark.parametrize("cfg", [make_test_param(CONFIG, "tu_leaks_info_leak_2")])
async def test_tu_leaks_info_leak_2(ws_client, cfg):
    """[TuLeaksInfo] Проверка сообщения об утечке типа TuLeaksInfoContent (Утечка 2: 181км)"""
    await _test_tu_leaks_info_leak_2(ws_client, cfg)


@pytest.mark.parametrize("cfg", [make_test_param(CONFIG, "output_signals_leak_2")])
async def test_output_signals_leak_2(ws_client, cfg):
    """[OutputSignalsInfo] Проверка выходных сигналов (Утечка 2: 181км)"""
