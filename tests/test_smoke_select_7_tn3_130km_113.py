...
from constants.expectations.select_7_expectations import Select7Expected as Exp
...
pytestmark = [
    ...
    pytest.mark.test_suite_data_id(13),
...
async def test_mask_signal_msg(ws_client):
    ...
    with allure.step("Снятие маскирования датчиков"):
        ...
        with allure.step(
            ...
            with allure.step("Маппинг данных в структуре"):
                parsed_payload = parser.parse_unmask_signal_msg(payload)
...
@pytest.mark.test_case_id("13")
@pytest.mark.offset(59.0)
@pytest.mark.asyncio
async def test_all_leaks_info(ws_client, imitator_start_time):
    ...
    with allure.step("Обработка сообщения об утечке типа AllLeaksInfoContent"):
        ...
        leak_wait_start_time, leak_wait_end_time = t_utils.get_leak_time_window(
            imitator_start_time,
            Exp.LEAK_START_INTERVAL,
            Exp.ALLOWED_TIME_DIFF_SECONDS,
...
@pytest.mark.test_case_id("14")
@pytest.mark.offset(59.0)
@pytest.mark.asyncio
async def test_tu_leaks_info(ws_client, imitator_start_time):
...
async def test_lds_status_during_leak(ws_client):
    ...
    in_neighbor_diagnostic_area = t_utils.find_diagnostic_area_by_id(flow_areas, Exp.IN_NEIGHBOR_DIAGNOSTIC_AREA_ID_VAL)
    out_neighbor_1_diagnostic_area = t_utils.find_diagnostic_area_by_id(
        flow_areas, Exp.OUT_NEIGHBOR_1_DIAGNOSTIC_AREA_ID_VAL
    )
    out_neighbor_2_diagnostic_area = t_utils.find_diagnostic_area_by_id(
        flow_areas, Exp.OUT_NEIGHBOR_2_DIAGNOSTIC_AREA_ID_VAL
    )
    with SoftAssertions() as soft_failures:
        StepCheck(
        ...
        StepCheck(
            f"Проверка режима работы СОУ на соседнем ДУ, id ДУ: {Exp.OUT_NEIGHBOR_1_DIAGNOSTIC_AREA_ID_VAL}",
            "ldsStatus",
            soft_failures,
        ).actual(out_neighbor_1_diagnostic_area.ldsStatus).expected(Exp.LDS_STATUS_DEGRADATION_VAL).equal_to()
        StepCheck(
            f"Проверка режима работы СОУ на соседнем ДУ, id ДУ: {Exp.OUT_NEIGHBOR_2_DIAGNOSTIC_AREA_ID_VAL}",
            "ldsStatus",
            soft_failures,
        ).actual(out_neighbor_2_diagnostic_area.ldsStatus).expected(Exp.LDS_STATUS_DEGRADATION_VAL).equal_to()
    ...
    with allure.step(f"Получение данных выходных сигналов для линейного участка с id: {Exp.LEAK_LINEAR_PART_ID_VAL}"):
        ...
        with allure.step("Получение обработанных данных выходных сигналов"):
            ...
            leak_wait_start_time, leak_wait_end_time = t_utils.get_leak_time_window(
                imitator_start_time,
                Exp.LEAK_START_INTERVAL,
                Exp.OUTPUT_ALLOWED_TIME_DIFF_SECONDS,
