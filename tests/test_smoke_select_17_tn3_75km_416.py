...
from constants.expectations.select_17_expectations import Select17Expected as Exp
...
pytestmark = [
    ...
    pytest.mark.test_suite_data_id(42),
...
@allure.description(
    ...
    f"т к для данных {Exp.TEST_SUITE_NAME_VAL} интенсивность утечки 13,2%.\n"
    "Примечание: тесты сообщений об утечке должны выполнятся раньше теста на квитирование"
)
@allure.title(f"[AllLeaksInfo] проверка начала утечки с 40 минуты на данных {Exp.TEST_SUITE_NAME_VAL}")
@allure.tag("subscribeAllLeaksInfoRequest")
@pytest.mark.test_case_id("42")
...
async def test_all_leaks_info(ws_client, imitator_start_time):
    ...
    with SoftAssertions() as soft_failures:
        ...
        ).expected(Exp.STATIONARY_STATUS_UNSTATIONARY_VAL).equal_to()
...
@allure.tag("SubscribeCommonSchemeRequest")
@pytest.mark.test_case_id("60")
@pytest.mark.offset(64.5)
@pytest.mark.asyncio
...
@pytest.mark.test_case_id("58")
@pytest.mark.offset(66.0)
@pytest.mark.asyncio
